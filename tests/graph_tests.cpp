...
TEST_CASE("Dijkstra shortestPath tests") {
    Graph graph(test_file);

    // SECTION("Shortest path found when a longer path exists") {
    //     REQUIRE(graph.getDijkstraPath(1, 3) == vector<size_t>{1, 3});
    // }

    // SECTION("Returns empty vector when no path exists") {
    //     REQUIRE(graph.getDijkstraPath(1, 2) == vector<size_t>{});
    // }

    // SECTION("Shortest path from a node to itself is itself") {
    //     REQUIRE(graph.getDijkstraPath(1, 1) == vector<size_t>{1});
    // }

    graph = Graph("./tests/dijkstra_graph.txt");

    SECTION("Shortest path avoids node with high indegree") {
        REQUIRE(graph.getDijkstraPath(5, 3) == vector<size_t>{5, 2, 3});
    }
}